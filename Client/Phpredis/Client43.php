...
class Client43 extends Redis
{
    ...
    public function __construct(array $parameters = array(), RedisLogger $logger = null)
    {
        ...
        $scheme = $parameters['scheme'] ?? null;

        if ($this->logger) {
            $this->logger->addAdapter($this);
            $this->logger->setScheme($this, $scheme);
        }
    }
    ...
    public function connect(
        ...
        $result = parent::connect($host, $port, $timeout, $reserved, $retry_interval,
            $read_timeout); // TODO: Change the autogenerated stub

        if ($result && $this->logger) {
            $this->logger->setIsConnected($this, true);
            $this->logger->setHost($this, $host);
            $this->logger->setPort($this, $port);
        }
